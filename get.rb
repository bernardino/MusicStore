...
class Get
	...
	def artistSongs(artist_id)
		return $db.select("	SELECT song_name, DECODE(alb_product_id, null, 'None', (SELECT album_name
	...
	def albumSongs(album_id)
		return $db.select("	SELECT song_number, song_name, song_length, rating, votes, current_price
							FROM song s, product p
